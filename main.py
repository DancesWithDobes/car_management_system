...
class CarManagementSystem(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.view_maintenance_button = QPushButton("View Maintenance")
        self.view_incident_button = QPushButton("View Incident")
    ...
    def update_buttons(self):
        selected_rows = self.cars_tablewidget.selectionModel().selectedRows()
        if selected_rows:
            self.remove_vehicle_button.setEnabled(True)
            self.view_maintenance_button.setEnabled(True)
            self.remove_maintenance_button.setEnabled(True)
            self.view_incident_button.setEnabled(True)
        else:
            self.remove_vehicle_button.setEnabled(False)
            self.view_maintenance_button.setEnabled(False)
            self.remove_maintenance_button.setEnabled(False)
            self.view_incident_button.setEnabled(False)

        self.add_maintenance_button.setEnabled(True)
        self.add_incident_button.setEnabled(True)
        self.remove_vehicle_button.setEnabled(True)
...
class MaintenanceDialog(QtWidgets.QDialog):
    ...
    def remove_selected_item(self):
        ...
        if selected_item:
            maintenance_type = selected_item.text()

            # Remove the item from the list widget
            self.maintenance_listwidget.takeItem(self.maintenance_listwidget.row(selected_item))

            # Remove the item from the database
            conn = sqlite3.connect("car_management.db")
            cursor = conn.cursor()

            # Get the car_id based on the license plate
            cursor.execute("SELECT id FROM cars WHERE license_plate = ?", (self.license_plate,))
            car_id = cursor.fetchone()[0]

            # Remove the maintenance record based on the car_id and maintenance_type
            cursor.execute("DELETE FROM maintenance WHERE car_id = ? AND maintenance_type = ?",
...
class IncidentDialog(QtWidgets.QDialog):
    ...
    def remove_selected_item(self):
        ...
        if selected_item:
            incident_description = selected_item.text()
            ...
            if car_record:
                ...
                cursor.execute("DELETE FROM incidents WHERE car_id = ? AND description = ?",
                               (car_id, incident_description))
                conn.commit()

                # Remove the item from the list widget
                self.incident_listwidget.takeItem(self.incident_listwidget.row(selected_item))
